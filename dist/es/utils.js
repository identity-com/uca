...
const resolveDefinition = (definition, definitions) => {
  if (_.isString(definition.type) && _.includes(getValidIdentifiers(definitions), definition.type)) {
    const innerDefinition = _.find(definitions, { identifier: definition.type });
...
const flagRequeredProps = (requiredArray, props) => _.map(props, p => _.merge(p, { required: _.includes(requiredArray, p.name) }));
